...
pub fn expr_parser<'src>(
    ...
    recursive(|expr| {
        ...
        let uint = text::int(10).map(|s: &str| s.parse::<u32>().unwrap());
        ...
        let bit_lit = choice((just('0').to(false), just('1').to(true)))
            .then_ignore(just('\'').not().rewind())
            .map_with(|b, e| Expr::Lit {
        ...
        let signal_ref = text::ident().then(bit_sel.or_not()).map_with(
            |(name, sel): (&str, Option<BitSel>), e| Expr::Ref {
                name: name.to_string(),
                sel,
                span: to_span(e.span()),
            },
        ...
        let primary = choice((parens, vec_lit, bit_lit, signal_ref)).padded();
        ...
        // eq_expr: unary (("==" | "!=") unary)?
        let eq_op = choice((just("==").to(0u8), just("!=").to(1u8))).padded();
        ...
        let and_expr = eq_expr.clone().foldl_with(
        ...
        let xor_expr = and_expr.clone().foldl_with(
        ...
        let or_expr = xor_expr.clone().foldl_with(
...
) -> impl Parser<'src, &'src str, Vec<Decl>, extra::Err<Rich<'src, char>>> + Clone {
...
pub fn stmt_parser<'src>(
    ...
    let lvalue = ident
        .then(
            uint.delimited_by(just('[').padded(), just(']').padded())
                .or_not(),
        )
